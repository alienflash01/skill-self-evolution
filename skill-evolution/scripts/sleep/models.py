"""Offline sleep engine — core data types.

These dataclasses are the interfaces between the sleep-cycle stages
(harvest -> mine -> replay -> consolidate -> stage -> adopt). They are
intentionally plain (pure stdlib dataclasses) so the package imports
cleanly on any Python 3.10+ interpreter and the deterministic mock path
...
class SessionDigest:
    ...
    Produced by ``harvest.digest_transcript`` from Claude Code transcript
    ...
    project: str = ""
    git_branch: str = ""
    started_at: str = ""            # ISO8601-ish timestamp from the transcript
    ended_at: str = ""
    user_prompts: List[str] = field(default_factory=list)
    assistant_finals: List[str] = field(default_factory=list)
    tools_used: List[str] = field(default_factory=list)
    files_touched: List[str] = field(default_factory=list)
    feedback_signals: List[str] = field(default_factory=list)  # "neg:...", "pos:..."
    n_user_turns: int = 0
    ...
    @classmethod
    def from_dict(cls, d: Dict[str, Any]) -> "SessionDigest":
...
class TaskRecord:
    ...
    # A stable hash keeps the same task in the same split across nights.
    split: str = "train"
    origin: str = "real"                    # real | dream (dream never enters val)
...
class ReplayResult:
    ...
    tokens: int = 0                 # approx tokens this rollout cost
    latency_ms: float = 0.0         # wall-clock for this rollout
...
class ConsolidationResult:
    """Outcome of one consolidation epoch (reflect -> bounded edit -> gate)."""

    accepted: bool
    gate_action: str                # accept_new_best | accept | reject | greedy_*
    baseline_score: float
    candidate_score: float
    new_skill: str
    new_memory: str
    applied_edits: List[EditRecord] = field(default_factory=list)
